...
// Lenis observing/re-asserting scroll position on top of native touch
// scrolling is a known source of stuck or dead gestures on real phones
// (rubber-banding fights the rAF-driven reposition). Coarse-pointer devices
// get pure native scrolling instead — touch momentum is already smooth
// there, so Lenis buys nothing but risk.
...
export function SmoothScroll({
  ...
  const nativeFallback = !!reduce || coarsePointer;
  ...
  const scrollTo = useCallback(
    ...
    [reduce, nativeFallback, nativeSource],
  );

  // Reduced motion and coarse-pointer (touch) devices drive the native
  // listener; only the fine-pointer, motion-safe path mounts Lenis and lets
  // LenisBridge feed the values instead.
